...
- [x] Add durable rate limiting for public feedback and AI operations. Migration `010_abuse_controls.sql` adds protected `operation_flags` and `rate_limit_buckets` tables with atomic security-definer functions. Public suggestions use a hashed-IP hourly limit and kill switch; AI generation uses a hashed-IP hourly limit, the existing per-user daily guard, and the same kill switch. Control failures fail closed. No new paid provider or Redis dependency was added. Tests and TypeScript checks pass.
