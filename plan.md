...
- [x] Repair authentication and ownership checks across all server actions. Profile, catalog, recommendations, saved jobs, tracker, generation, dashboard, suggestions, and PDF paths were reviewed; the current controls and remaining gates are recorded in `docs/server-action-audit.md`.
...
- [x] Complete every applicable item in videcodingsecurity.txt. Supabase security advisors report zero current lints after migrations `004_lock_ingestion_health`, `005_restrict_rls_helper`, `006_revoke_rls_helper_roles`, and explicit deny policies in `011_lock_abuse_controls`.
- [x] Scan files and history for secrets. No real credential matches were found; historical matches were credential-shaped README placeholders and are documented in `docs/security-scan.md`.
...
- [x] Add durable rate limiting for public feedback and AI operations. Migrations `010_abuse_controls.sql` and `011_lock_abuse_controls.sql` add protected `operation_flags` and `rate_limit_buckets` tables with atomic security-definer functions and explicit public-role deny policies. Public suggestions use a hashed-IP hourly limit and kill switch; AI generation uses a hashed-IP hourly limit, the existing per-user daily guard, and the same kill switch. Control failures fail closed. No new paid provider or Redis dependency was added. Tests and TypeScript checks pass; the live security advisor is zero-lint.
