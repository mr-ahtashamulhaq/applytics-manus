...
const TextPressure = ({
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => { cursorRef.current = { x: e.clientX, y: e.clientY } }
  ...
  const cls = ['tp-title', className, flex ? 'flex justify-between' : '', stroke ? 'tp-stroke' : ''].filter(Boolean).join(' ')
